function [t_epoch,...
          state_epoch,...
          NOT_YET_STABLE,...
          cov_max_gs] = runLiuModel_withADHR_toSS(conductance_set,... % int or vec
                                          epoch_length,...         % in [ms]
                                          max_time,...             % in [ms]
...
tMax = epoch_length;  % Simulate without ADHR for one epoch
dt   = 0.02;          % [ms]
res  = 1;             % [ms]
ADHR_switch = 0; % boolean (1 if ON, any other value sets to off)
simulationParams = [tMax; dt; res; ADHR_switch;adhr_time_constant];

[t_epoch,state_epoch] = mexLiuModel(simulationParams,gVec,sensorVec,regMap);
%%%%%%%%%%%%%%%%%%%%%%%%%%%

NOT_YET_STABLE = true;
while NOT_YET_STABLE
  %%% SECOND SIMULATION - ADHR ON %%%
  tMax = tMax+epoch_length;  % [ms]
  dt   = 0.02;          % [ms]
  res  = 1;             % [ms]
  ADHR_switch = 1; % boolean (1 if ON, any other value sets to off)
  simulationParams = [epoch_length; dt; res; ADHR_switch;adhr_time_constant];

  [t_epoch,state_epoch] = mexLiuModel(simulationParams,gVec,sensorVec,regMap,state_epoch(end,:)');
  %%%%%%%%%%%%%%%%%%%%%%%%%%%
  
  max_gs = state_epoch(:,3:3:25);
  cov_max_gs = max(std(max_gs)./mean(max_gs));
  abs_diff_gs = max(max(max_gs)-min(max_gs));
  if cov_max_gs < 0.015 && abs_diff_gs < 3
    NOT_YET_STABLE = false;
  end
  if tMax > max_time
    break
  end
...
if plot_verbosity > 0
  ...
  max_g_Na = state_epoch(:,3);
  max_g_CaT = state_epoch(:,6)*30;
  max_g_CaS = state_epoch(:,9)*30;
  max_g_A = state_epoch(:,12);
  max_g_KCa = state_epoch(:,15);
  max_g_Kd = state_epoch(:,18);
  max_g_H = state_epoch(:,21)*30;
  g_leak = state_epoch(:,24);
  G = [max_g_Na, max_g_CaT, max_g_CaS, max_g_A, max_g_KCa, max_g_Kd, max_g_H, g_leak];
    
  figure;
  subplot(2,1,1)
  plot(t_epoch/1000,state_epoch(:,1))
